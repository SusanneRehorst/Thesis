...
plot1<- ggplot(DOC_Quantity, aes(x = sentences, fill = M)) + 
...
model2 <- lm(sentences ~ M+DOC_TYPE, data = DOC_Quantity)
...
## Average readability between documents: Mandatory or Voluntary, subset by DOC_TYPE (ESG)
After examining the average readability score between mandotory and voluntary, this difference is examined in more detail by making a distinction in DOC_TYPE. So, if it is an ESG, G, or E document.  
```{r}
...
Variable transformation is also done to see which of the 5 largest international voluntary frameworks (cdp cdsb gri iirc sasb) are mentioned the most across the current reporting standards. Also, the mentioning of sustainable development goals (SDGs) is counted and the amount of TCFD mentions (tcfd is a special framework for climate related financial disclosure).
```{r}
...
The numbers can also be visualized in a bar chart, where average (relative) frequency per 1000 words is shown per each word category, per Mandatory/Voluntary
```{r}
ggplot(R_Data, aes(factor(M), average_restriction, fill=M)) + 
...
After examining the average readability score between mandatory and voluntary, this difference is examined in more detail by making a distinction in DOC_TYPE. So, if it is an ESG, G, or E document. Because the relative frequency is used, differences in document size do not influence the measurement. 
```{r}
R_DOC <- R_Data %>%
  group_by(DOC_TYPE, M) %>% 
  summarize(average_negative_strong=mean(negative_strong),
...
ermodelx14 <- lm(retriction~DOC_TYPE, data = M_R_DOC)
...
#frameworks separated
fermodelx11 <- lm(cdp~DOC_TYPE, data = M_R_DOC)
fermodelx12 <- lm(cdsb~DOC_TYPE, data = M_R_DOC)
fermodelx13 <- lm(gri~DOC_TYPE, data = M_R_DOC)
fermodelx14 <- lm(sasb~DOC_TYPE, data = M_R_DOC)
fermodelx15 <- lm(tcfd~DOC_TYPE, data = M_R_DOC)
fermodelx16 <- lm(iirc~DOC_TYPE, data = M_R_DOC)
fermodelx17 <- lm(sdg~DOC_TYPE, data = M_R_DOC)
...
ermodelyx14 <- lm(retriction~DOC_TYPE, data = V_R_DOC)
...
M_RES_DOC <- Group_R[Group_R$M=="1" & Group_R$GROUP_TYPE=="ESG" & Group_R$GROUP_TYPE=="G", ]
V_RES_DOC <- Group_R[Group_R$M=="0" & Group_R$GROUP_TYPE=="ESG" & Group_R$GROUP_TYPE=="E", ]
...
ermodeldx14 <- lm(retriction~GROUP_TYPE, data = M_RES_DOC)
...
#frameworks separated
fermodeldx11 <- lm(cdp~GROUP_TYPE, data = M_RES_DOC)
fermodeldx12 <- lm(cdsb~GROUP_TYPE, data = M_RES_DOC)
fermodeldx13 <- lm(gri~GROUP_TYPE, data = M_RES_DOC)
fermodeldx14 <- lm(sasb~GROUP_TYPE, data = M_RES_DOC)
fermodeldx15 <- lm(tcfd~GROUP_TYPE, data = M_RES_DOC)
fermodeldx16 <- lm(iirc~GROUP_TYPE, data = M_RES_DOC)
fermodeldx17 <- lm(sdg~GROUP_TYPE, data = M_RES_DOC)
...
ermodelkyx14 <- lm(retriction~GROUP_TYPE, data = V_RES_DOC)
