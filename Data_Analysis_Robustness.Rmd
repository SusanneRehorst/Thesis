...
# Data Analysis H3
In the documents Data_Processing_Regulations.Rmd and Data_Processing_Metrics.Rmd, the data was cleaned and processed. In this markdown, robustness analysis will be performed on this processed data for Hypothesis 3 (Document similarity) by using a variation of Processed_Data as input. 
...
# Hypothesis 3: Robustness checks

## Robustness: Filter POS tags
Check 1: As a robustness check, only nouns and adverbs are selected instead of also including verbs and adverbs. 
```{r}
...
Check 3: Applying stemming to the sample instead lemmatization.
```{r}
library(SnowballC)
Processed_Data <- wordStem(Processed_Data$lemma, language = "porter")
```

Run each of the checks separately and compare results with results of Data_Analysis_H3.Rmd document. 
