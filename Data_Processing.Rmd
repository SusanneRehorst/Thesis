...
# Data Processing
This R Markdown contains the steps to load in the data and applying pre-processing steps, which are required for text the analysis

## Loading in Data
For every ESG regulation, the text is loaded in from a .txt file. Subsequently, every regulation is stored as a string value in one cell. Thereafter, texts are merged in one character string and a document ID is created, which can be used for merging different data sets later on. The sample also already contains ESG data document (DOC_ID=..)
```{r warning=FALSE}
...
## Merging Data
Mandatory E, S, or G regulations are merged if they come from the same sources and apply to the same region. Because guidelines that cover all ESG aspects are compared to regulations that sometimes only cover E, S, or G. Although E,S,G are not added, there is merging on region level if the regulations apply to the same category (i.e., E, S, or G). This results in a group of ESG mandatory, ESG voluntary, G mandatory, E mandatory, S mandatory; which are all on region level. 
```{r}
...
# Final cleaning to make sure all punctuation except dots is removed
...
Quantity_Data$DOC_ID <- 1:nrow(Quantity_Data) # Add ID
rm(Vector_Data_Q)
...
Measure in each document the occurrence of a pre-selected list of words: 
```{r}
...
Restriction_Data$framework <- sum(cdp,cdsb,gri,iirc,sasb,tcfd)
...
Merge the Quantity data with the Restrictive data and the Meta Data, based on Document ID. This will make it easier to analyze the data later on. 
```{r}
Meta_Data <- read_delim("Meta_Data.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)

Descriptive_Data <- merge(Quantity_Data,Restriction_Data,by=DOC_ID)
rm(Quantity_Data)
rm(Restriction_Data)

Descriptive_Data <- merge(Descriptive_Data,Meta_Data,by=DOC_ID)
rm(Meta_Data)
...
```{r}
Processed_Data <- udpipe_annotate(udmodel_english, Vector_Data, doc_id = DOC_ID, tokenizer = "tokenizer", tagger = "default", parser = "none")
Processed_Data <- as.data.frame(Processed_Data)
Processed_Data <- Processed_Data %>% select(doc_id, token_id, lemma, upos)

rm(Vector_Data)  # Check if abbreviations like CDP, GRI are maintained)
```

## Filter out tokens based on POS
Only "open class" words are maintained. These are verbs, adverbs, nouns, proper nouns, adjectives, and auxiliary. Words that couldn't be assigned "X" are reassigned or dropped. 
```{r}
